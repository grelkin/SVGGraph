...
*/
    protected function FindAxisTextBBox($length_x, $length_y, $x_axes, $y_axes)
    {
        ...
        $div_size  = $this->DivisionOverlap($x_axes, $y_axes);
        $inside_x  = ('inside' == $this->GetFirst(
                $this->axis_text_position_h,
        ...
        if ($this->show_axis_text_h) {
            ...
            $positions = $this->XAxisTextPositions(
                $points,
                $offset,
                $div_size['b'],
                $this->axis_text_angle_h,
                $inside_x
            );
        ...
        if ($this->show_axis_text_v) {
            ...
            foreach ($y_axes as $y_axis) {
                ...
                $positions   = $this->YAxisTextPositions(
                    $points,
                    $div_size[$right ? 'r' : 'l'],
                    $offset,
                    $this->ArrayOption($this->axis_text_angle_v, $axis_no),
                    $inside_y xor $right,
                    $axis_no
                );
    ...
    protected function DivisionOverlap($x_axes, $y_axes)
    {
        ...
        if ($this->show_divisions || $this->show_subdivisions) {
            ...
            for ($i = 0; $i < $x_count; ++$i) {
                ...
                $dx = $this->DOverlap(
                    $this->GetFirst(
                        $this->ArrayOption($this->division_style_h, $i),
                        $this->division_style
                    ),
                    $this->GetFirst(
                        $this->ArrayOption($this->division_size_h, $i),
                        $this->division_size
                    )
                );
                $sx = $this->DOverlap(
                    $this->GetFirst(
                        $this->ArrayOption($this->subdivision_style_h, $i),
                        $this->subdivision_style
                    ),
                    $this->GetFirst(
                        $this->ArrayOption($this->subdivision_size_h, $i),
            ...
            for ($i = 0; $i < $y_count; ++$i) {
                ...
                $dy = $this->DOverlap(
                    $this->GetFirst(
                        $this->ArrayOption($this->division_style_v, $i),
                        $this->division_style
                    ),
                    $this->GetFirst(
                        $this->ArrayOption($this->division_size_v, $i),
                        $this->division_size
                    )
                );
                $sy = $this->DOverlap(
                    $this->GetFirst(
                        $this->ArrayOption($this->subdivision_style_v, $i),
                        $this->subdivision_style
                    ),
                    $this->GetFirst(
                        $this->ArrayOption($this->subdivision_size_v, $i),
                        $this->subdivision_size
                    )
                );
                $y  = max($dy, $sy);
...
*/
    protected function GetAxes($ends, &$x_len, &$y_len)
    {
        ...
        for ($i = 0; $i < $x_axis_count; ++$i) {
            ...
            if ($this->ArrayOption($this->log_axis_y, $i) && $this->flip_axes) {
                $x_axis = new AxisLog(
                    $x_len, $max_h, $min_h, $x_min_unit, $x_fit,
                ...
                $x_axis = new AxisFixed(
                    $x_len, $max_h, $min_h, $grid_division,
                    $x_units_before, $x_units_after, $x_decimal_digits, $x_text_callback
                );
            }
            $x_axes[] = $x_axis;
        ...
        for ($i = 0; $i < $y_axis_count; ++$i) {
            ...
            if ($this->ArrayOption($this->log_axis_y, $i) && !$this->flip_axes) {
                $y_axis = new AxisLog(
                    $y_len, $max_v, $min_v, $y_min_unit, $y_fit,
                ...
                $y_axis = new AxisFixed(
                    $y_len, $max_v, $min_v, $grid_division,
                    $y_units_before, $y_units_after, $y_decimal_digits, $y_text_callback
                );
            }
    ...
    protected function CalcAxes()
    {
        ...
        list($x_axes, $y_axes) = $this->GetAxes(
            $ends,
            $this->g_width,
            $this->g_height
        );
...
*/
    protected function GetGridPointsY($axis)
    {
        ...
        return $this->y_axes[$axis]->GetGridPoints(
            $min_space_v,
...
*/
    protected function Labels($axis_text = '')
    {
        ...
        if (!empty($this->label_h) || !empty($this->label_v)) {
            ...
            $label_text['fill'] = $this->GetFirst(
                $this->label_colour_h,
                $this->label_colour,
                $this->axis_text_colour_h,
                $this->axis_text_colour
            );
    ...
    protected function Axes()
    {
        ...
        if ($this->show_axis_text_v) {
            for ($i = 0; $i < $y_count; ++$i) {
                ...
                if (!is_null($axis)) {
                    ...
                    $axis_text .= $this->YAxisText(
                        $points,
                        $text_offset[$i > 0 ? 'r' : 'l'],
                        $offset,
                        $this->ArrayOption($this->axis_text_angle_v, $i),
                        $i > 0,
                        $i
                    );
                }
        ...
        if ($this->show_divisions) {
            ...
            if ($this->show_axis_h) {
                ...
                if (!empty($dx_path)) {
                    $dx_colour = $this->GetFirst(
                        $this->division_colour_h,
            ...
            if ($this->show_axis_v) {
                for ($i = 0; $i < $y_count; ++$i) {
                    if (!is_null($this->y_axes[$i])) {
                        $points  = $this->GetGridPointsY($i);
                        $dy_path = $this->YAxisDivisions(
                            $points,
                            $i > 0 ? $this->g_width : $xoff,
                            false,
                            $i
                        );
                        if (!empty($dy_path)) {
            ...
            if ($this->show_subdivisions) {
                if ($this->show_axis_h) {
                    ...
                    $sdx_path = $this->XAxisDivisions(
                        $subdivs,
                        $this->GetFirst(
                            $this->subdivision_style_h,
                            $this->subdivision_style
                        ),
                        $this->GetFirst(
                            $this->subdivision_size_h,
                            $this->subdivision_size
                        ),
                        $yoff
                    );
                    ...
                    if (!empty($sdx_path)) {
                        $sdx_colour = $this->GetFirst(
                            $this->subdivision_colour_h,
...
*/
    protected function GridCrossHairs(&$grid_group)
    {
        ...
        if ($this->crosshairs_show_h) {
            ...
            $hch['stroke-width'] = $this->GetFirst(
                $this->crosshairs_stroke_width_h,
                $this->crosshairs_stroke_width
            );
            $opacity             = $this->GetFirst(
                $this->crosshairs_opacity_h,
        ...
        if ($this->crosshairs_show_v) {
            ...
            $hch['stroke-width'] = $this->GetFirst(
                $this->crosshairs_stroke_width_v,
                $this->crosshairs_stroke_width
            );
            $opacity             = $this->GetFirst(
                $this->crosshairs_opacity_v,
                $this->crosshairs_opacity
            );
            if ($opacity > 0 && $opacity < 1) {
                $hch['opacity'] = $opacity;
        ...
        $text = $this->Element(
            ...
            $text_group,
            null,
            $this->Element('rect', $text_rect) . $this->Text(
                '',
                $font_size,
                $text_element
            )
    ...
    protected function Grid()
    {
        ...
        if ($this->show_grid_subdivisions) {
            ...
            if ($subpath_h != '' || $subpath_v != '') {
                $colour_h = $this->GetFirst(
                    $this->grid_subdivision_colour_h,
                    $this->grid_subdivision_colour,
                    $this->grid_colour_h,
                    $this->grid_colour
                );
                $colour_v = $this->GetFirst(
                    $this->grid_subdivision_colour_v,
                    $this->grid_subdivision_colour,
                    $this->grid_colour_v,
                    $this->grid_colour
                );
                $dash_h   = $this->GetFirst(
                    $this->grid_subdivision_dash_h,
                    $this->grid_subdivision_dash,
                    $this->grid_dash_h,
                    $this->grid_dash
                );
                $dash_v   = $this->GetFirst(
                    $this->grid_subdivision_dash_v,
                    $this->grid_subdivision_dash,
                    $this->grid_dash_v,
                    $this->grid_dash
                );
...
*/
    protected function Guidelines($depth)
    {
        ...
        $textopts     = array(
            ...
            'fill'        => $this->GetFirst(
                $this->guideline_text_colour,
                $this->guideline_colour
            ),
        );
        $text_opacity = $this->GetFirst(
            $this->guideline_text_opacity,
...
*/
    protected function BuildGuideline(&$line, &$lines, &$text, &$path, &$d)
    {
        ...
        if (!empty($line['title'])) {
            ...
            list($text_w, $text_h) = $this->TextSize(
                $line['title'],
                $font_size,
                $font_adjust,
                $this->encoding,
